...
def test_create_game(test_db):
    try:
        ...
        game = test_db.query(GameTable).filter_by(id_game=game_id).first()
        assert game is not None
        assert game_id == game.id_game
...
def test_get_game(test_db):
    try:
        ...
        game = test_db.query(GameTable).filter_by(id_game=game_id).first()
        assert game == game_queries.get_game(game_id, test_db)
    except IntegrityError:
        pass
...
def test_listing_games(test_db): #Chequear
    try:
        ...
        assert games == game_queries.listing_games(test_db)
    except IntegrityError:
        pass
...
def test_remove_player(test_db):
    ...
    game_queries.remove_player(game_id, test_db)
    assert game_queries.get_players(game_id, test_db) == 0
...
def test_remove_game(test_db):
    try:
        ...
        assert test_db.query(GameTable).filter_by(id_game=game_id).first() is None
    except IntegrityError:
        pass
