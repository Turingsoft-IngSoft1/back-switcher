...
def test_create_game(test_db):
    try:
        game_id = game_queries.create_game("Draken", 4, 2, "password", test_db)
...
def test_get_game(test_db):
    try:
        game_id = game_queries.create_game("Europas evija hjarta", 4, 2, "password", test_db)
        game = test_db.query(GameTable).filter_by(id=game_id).first()
        assert game_queries.get_game(game_id, test_db) == Game(id=1, name='Europas evija hjarta', state='Waiting', turn=0, host=0, players=1, max_players=4, min_players=2, password='password', moves_deck=50)
    except IntegrityError:
        pass

def test_set_game_state(test_db):
    
    game_id = game_queries.create_game("Genom Sverige i tiden", 3, 2, "password", test_db)
...
def test_get_game_state(test_db):
    try:
        game_id = game_queries.create_game("Västkustens styrka och stolthet", 3, 2, "password", test_db)
        game_queries.set_game_state(game_id, "Playing", test_db)
        game = test_db.query(GameTable).filter_by(id=game_id).first()
        assert game.state == "Playing"
    except IntegrityError:
...
def test_listing_games(test_db): #Chequear
    try:
        game_id = game_queries.create_game("Gottland regemente", 3, 2, "password", test_db)
        game_id2 = game_queries.create_game("Skaraborgs regemente", 4, 2, "password", test_db)
        game_id3 = game_queries.create_game("Uppland regemente", 2, 2, "password", test_db)
        games = test_db.query(GameTable).all()
        assert game_queries.listing_games(test_db) == [Game(id=1, name='Gottland regemente', state='Waiting', turn=0, host=0, players=1, max_players=3, min_players=2, password='password', moves_deck=50), Game(id=2, name='Skaraborgs regemente', state='Waiting', turn=0, host=0, players=1, max_players=4, min_players=2, password='password', moves_deck=50), Game(id=3, name='Uppland regemente', state='Waiting', turn=0, host=0, players=1, max_players=2, min_players=2, password='password', moves_deck=50)]
    except IntegrityError:
        pass

def test_set_game_turn(test_db):
    
    game_id = game_queries.create_game("Scania regemente", 3, 2, "password", test_db)
    game = test_db.query(GameTable).filter_by(id=game_id).first()
...
def test_get_game_turn(test_db):
    try:
        game_id = game_queries.create_game("Blekinge regemente", 3, 2, "password", test_db)
        game_queries.set_game_turn(game_id, 2, test_db)
...
def test_set_game_host(test_db):
    try:
        game_id = game_queries.create_game("Jämtland regemente", 3, 2, "password", test_db)
        game = test_db.query(GameTable).filter_by(id=game_id).first()
...
def test_get_players(test_db):
    try:
        game_id = game_queries.create_game("Värmlands regemente", 3, 2, "password", test_db)
...
def test_get_max_players(test_db):
    try:
        game_id1 = game_queries.create_game("Alvsborg regemente", 3, 2, "password", test_db)
        game_id2 = game_queries.create_game("Dalregemente", 4, 2, "password", test_db)
...
def test_get_min_players(test_db):
    try:
        game_id1 = game_queries.create_game("Hallands regemente", 3, 2, "password", test_db)
        game_id2 = game_queries.create_game("Västgöta regemente", 4, 3, "password", test_db)
...
def test_add_player(test_db):
    try:
        game_id = game_queries.create_game("Gotlands regemente", 3, 2, "password", test_db)
        game_queries.add_player(game_id, test_db)
        assert game_queries.get_players(game_id, test_db) == 2
        game_queries.add_player(game_id, test_db)
        assert game_queries.get_players(game_id, test_db) == 3
